...
class _MultiplayerQuizScreenState extends State<MultiplayerQuizScreen>
    ...
    Widget columnContent = Column(
      children: [
        ...
        if (!_showResults) QuizBottomBar(
          ...
          questionId: isPlayer1 ? _player1QuizState.question.id : _player2QuizState.question.id,
          onSkipPressed: () => _handleSkipForPlayer(isPlayer1),
          onUnlockPressed: () => _handleUnlockBiblicalReferenceForPlayer(isPlayer1),
  ...
  Widget _buildScrollablePlayerArea(BuildContext context, {required bool isPlayer1, required String playerName}) {
    ...
    final showingBiblicalReference = isPlayer1 ? _player1ShowingBiblicalReference : _player2ShowingBiblicalReference;
    final biblicalReference = isPlayer1 ? _player1BiblicalReference : _player2BiblicalReference;
    ...
    return Container(
      ...
      child: showingBiblicalReference && biblicalReference != null
          ? _buildBiblicalReferenceFullScreen(context, biblicalReference, isPlayer1)
          : Column(
              children: [
                Expanded(
  ...
  Widget _buildBiblicalReferenceFullScreen(BuildContext context, String biblicalReference, bool isPlayer1) {
    return Scaffold(
      appBar: AppBar(
      ...
      body: Container(
        ...
        child: Column(
          children: [
            Expanded(
              child: _buildBiblicalContentForPlayer(isPlayer1, biblicalReference),
            ),
  ...
  Widget _buildBiblicalContentForPlayer(bool isPlayer1, String biblicalReference) {
    final isLoading = isPlayer1 ? _player1BiblicalLoading : _player2BiblicalLoading;
    final content = isPlayer1 ? _player1BiblicalContent : _player2BiblicalContent;
  ...
  Future<void> _loadBiblicalReferenceForPlayer(String biblicalReference, bool isPlayer1) async {
    try {
      ...
      if (mounted) {
        setState(() {
          if (isPlayer1) {
            _player1BiblicalError = 'Netwerkfout. Controleer uw internetverbinding.';
            _player1BiblicalLoading = false;
          } else {
            _player2BiblicalError = 'Netwerkfout. Controleer uw internetverbinding.';
            _player2BiblicalLoading = false;
          }
        });
      }
      ...
      if (errorMessage.contains('Time-out')) {
        ...
        if (mounted) {
          setState(() {
            if (isPlayer1) {
              _player1BiblicalError = 'Time-out bij het laden van de bijbeltekst. Probeer het later opnieuw.';
              _player1BiblicalLoading = false;
            } else {
              _player2BiblicalError = 'Time-out bij het laden van de bijbeltekst. Probeer het later opnieuw.';
              _player2BiblicalLoading = false;
            }
          });
        }
