...
class PlatformFeedbackService {
  ...
  void _detectPlatform() {
    ...
    AppLogger.info(
        'Platform detected: $_currentPlatform, feedback multiplier: $_platformMultiplier');
  }
  ...
  Duration getStandardizedFeedbackDuration({bool slowMode = false}) {
    ...
    AppLogger.debug(
        'Feedback duration: base=${slowMode ? _slowDurationMs : _normalDurationMs}, adjusted=$adjustedDuration (platform: $_currentPlatform)');
  ...
  void setFeedbackDurations(
      {required Duration normalDuration, Duration? slowDuration}) {
    assert(!normalDuration.isNegative && normalDuration.inMilliseconds > 0);
    _normalDurationMs = normalDuration.inMilliseconds;
    _slowDurationMs =
        slowDuration?.inMilliseconds ?? (normalDuration.inMilliseconds * 2);
  }
  ...
  Map<String, dynamic> getPlatformInfo() {
    return {
      ...
      'slowModeDuration':
          getStandardizedFeedbackDuration(slowMode: true).inMilliseconds,
    };
  }
